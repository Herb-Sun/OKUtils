...
NSArray *OKAllFilesByPath(NSString *path)
{
    NSError *error = nil;
    //fileList便是包含有该文件夹下所有文件的文件名及文件夹名的数组
    NSArray *fileList = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:path error:&error];
    if (error || fileList.count == 0) { return @[]; }
    
    return fileList;
}

BOOL OKCreateFile(NSString *path, NSString *fileName)
{
...
BOOL OKCreateDirectory(NSString *path, NSString *dirName)
{
    NSString *dirPath = [path stringByAppendingPathComponent:dirName];
    
    BOOL isDir = NO;
    BOOL isExits = [[NSFileManager defaultManager] fileExistsAtPath:dirPath isDirectory:&isDir];
    
    if (isExits && isDir) return YES;
...
unsigned long long OKFolderSize(NSString *folderPath)
{
...
unsigned long long OKApplicationSize(void)
{
    unsigned long long documentSize = OKFolderSize(OKDocumentsPath());
    unsigned long long librarySize = OKFolderSize(OKLibraryPath());
    unsigned long long cacheSize = OKFolderSize(OKCachesPath());
    return documentSize + librarySize + cacheSize;
}

NSData *OKReadFile(NSString *path, NSString *fileName)
{
    NSData *data;
    if (fileName && path) {
...
void OKWriteFile(NSData *data, NSString *path, NSString *fileName)
{
    if (data && fileName && path) {
        
        NSString *filePath = [path stringByAppendingPathComponent:fileName];
        OKCreateFile(path, fileName);
...
void OKAppendFile(NSData *data, NSString *path, NSString *fileName)
{
    if (data && fileName && path) {
